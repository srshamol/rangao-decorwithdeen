...
import { 
  ...
  MessageSquare, User, Eye, EyeOff, Lock, Headset, LineChart, Shield, Sparkles, Palette, Image
} from "lucide-react";
      ...
      {hasSub ? (
        <div className="space-y-1">
          <button 
            ...
            className={`w-full flex items-center ${isSidebarCollapsed && !isMobileOpen ? 'justify-center' : 'justify-between px-3'} py-2.5 rounded-xl text-sm font-medium transition-all group ${isSubActive ? "bg-white/15 text-white" : "text-white/60 hover:bg-white/10 hover:text-white"}`}
          >
            <div className="flex items-center gap-3">
              <div className={`w-7 h-7 rounded-xl flex items-center justify-center transition-all ${isSubActive ? "bg-white/20 text-white" : "text-white/50 group-hover:text-white"}`}>
                <item.icon size={15} /> 
              </div>
              {(!isSidebarCollapsed || isMobileOpen) && <span>{item.label}</span>}
            </div>
            {(!isSidebarCollapsed || isMobileOpen) && <ChevronDown size={12} className={`transition-transform duration-300 text-white/40 ${localIsOpen ? "rotate-0" : "-rotate-90"}`} />}
          </button>
          
          <AnimatePresence>
            {localIsOpen && (!isSidebarCollapsed || isMobileOpen) && (
              <motion.div 
                initial={{ height: 0, opacity: 0 }} 
                animate={{ height: "auto", opacity: 1 }} 
                exit={{ height: 0, opacity: 0 }} 
                className="ml-5 pl-3 border-l border-white/10 space-y-0.5"
              >
                {item.subItems!.map((sub: SidebarSubItem, sIdx: number) => {
                  ...
                  return (
                    <Link 
                      key={sIdx} 
                      href={sub.href} 
                      className={`flex items-center justify-between px-3 py-2 rounded-xl text-xs font-medium transition-all relative group ${isSubItemActive ? "text-white bg-white/15" : "text-white/60 hover:text-white hover:bg-white/10"}`}
                    >
                      <div className="flex items-center gap-2.5">
                        <div className={`w-1.5 h-1.5 rounded-xl shrink-0 ${isSubItemActive ? "bg-white" : "bg-white/20 group-hover:bg-white/50"}`} />
                        <span>{sub.label}</span>
                      </div>
                      {(sub.badge ?? 0) > 0 && (
                        <span className="bg-white/20 text-white px-2 py-0.5 rounded-xl text-[9px] font-bold">
                          {sub.badge}
                        </span>
                      )}
                    </Link>
        ...
        <Link 
          href={item.href || "#"} 
          className={`flex items-center ${isSidebarCollapsed && !isMobileOpen ? 'justify-center px-0' : 'gap-3 px-3'} py-2.5 rounded-xl text-sm font-medium transition-all group relative ${isActive ? "bg-white/15 text-white" : "text-white/60 hover:bg-white/10 hover:text-white"}`}
        >
          <div className={`w-7 h-7 rounded-xl flex items-center justify-center transition-all shrink-0 ${isActive ? "bg-white/20 text-white" : "text-white/50 group-hover:text-white"}`}>
            <item.icon size={15} /> 
          </div>
          {(!isSidebarCollapsed || isMobileOpen) && (
            <>
              <span className="flex-1">{item.label}</span>
              {isActive && <div className="w-1.5 h-1.5 bg-emerald-300 rounded-xl shrink-0" />}
            </>
          )}
          
          {isSidebarCollapsed && !isMobileOpen && (
            <div className="absolute left-full ml-3 px-3 py-1.5 bg-slate-900 text-white text-[10px] font-semibold rounded-xl opacity-0 group-hover:opacity-100 transition-all duration-200 pointer-events-none whitespace-nowrap z-[100] shadow-xl">
              {item.label}
...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  const [isSidebarOpen, setIsSidebarOpen] = useState<Record<string, boolean>>({});
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    // Check if identifier is likely a phone number or username (doesn't contain @)
    if (!identifier.includes("@")) {
      const { data: profile, error: profileError } = await supabase
        .from('staff_profiles')
        .select('email')
        .eq('phone' as any, identifier)
        .single();
    ...
    return (
      <div className="min-h-screen flex w-full bg-[#f8fafc] dark:bg-[#050505]">
        ...
        <div className="hidden lg:flex flex-col justify-between w-[45%] bg-[#ebf3ee] dark:bg-[#0a1510] relative overflow-hidden p-12">
          ...
          <div className="absolute top-20 -left-20 w-64 h-64 bg-[#1b6038]/5 rounded-xl blur-3xl" />
            ...
            <div className="space-y-6">
              {[
                ...
                <div key={i} className="flex items-start gap-4">
                  <div className="w-10 h-10 rounded-xl bg-white dark:bg-emerald-900/30 flex items-center justify-center text-[#1b6038] dark:text-emerald-400 shrink-0 shadow-sm border border-black/5 dark:border-white/5">
                    <f.icon size={20} />
                  </div>
                  <div>
        ...
        <div className="flex-1 flex flex-col items-center justify-center p-6 relative">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-[420px]">
            <div className="bg-white dark:bg-[#0c0c0c] rounded-xl p-8 sm:p-10 shadow-[0_20px_60px_-15px_rgba(0,0,0,0.05)] dark:shadow-none border border-slate-100 dark:border-white/5 relative z-10">
              
              <div className="flex flex-col items-center mb-8 text-center">
                <div className="w-16 h-16 bg-[#ebf3ee] dark:bg-emerald-900/30 rounded-xl flex items-center justify-center mb-5 border border-[#1b6038]/10">
              ...
              <form onSubmit={handleLogin} className="space-y-5">
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      placeholder={t("email_mobile_placeholder")}
                    />
                  </div>
                </div>
                ...
                <div className="flex items-center justify-between pt-1">
                  <label className="flex items-center gap-2 cursor-pointer group">
                    <input type="checkbox" className="w-4 h-4 rounded-xl border-slate-300 text-[#1b6038] focus:ring-[#1b6038]" />
                ...
                {error && <div className="p-3 bg-rose-50 border border-rose-200 rounded-xl text-rose-600 text-xs font-bold text-center">{error}</div>}
  ...
  const navGroups = [
    {
      ...
      items: [
        ...
        { href: "/admin/media", label: t("media_library"), icon: Image },
    ...
    },
    {
      ...
      items: [
        { href: "/admin/store-customize", label: t("store_customize"), icon: Palette },
  ...
  return (
    <div className="h-screen bg-[#f8fafc] dark:bg-[#050505] flex overflow-hidden selection:bg-primary/20 selection:text-primary transition-colors duration-500">
      ...
      <motion.aside 
        initial={false}
        animate={{ 
          width: isMobileOpen ? 300 : (isSidebarCollapsed ? 100 : 280), 
          x: 0, 
          left: isMobileOpen ? 0 : (isMounted && typeof window !== 'undefined' && window.innerWidth < 1024 ? -320 : 0)
        }}
        className={`fixed inset-y-0 left-0 z-50 bg-[#0F3D2E] lg:sticky lg:top-0 flex flex-col h-screen border-r border-primary/20 transition-all duration-700 ease-[cubic-bezier(0.23,1,0.32,1)] shadow-xl`}
      >
        <div className={`flex flex-col h-full ${isSidebarCollapsed && !isMobileOpen ? 'overflow-visible' : 'overflow-hidden'}`}>
          ...
          <div className="px-3 py-4 border-b border-white/10">
            <div className={`flex items-center ${isSidebarCollapsed && !isMobileOpen ? 'justify-center' : 'justify-between'}`}>
              <Link href="/admin" className="flex items-center gap-3">
                {gen.favicon ? (
                  <div className="w-9 h-9 rounded-xl overflow-hidden bg-white/20 flex items-center justify-center shrink-0">
                    <img src={gen.favicon} alt="" className="w-7 h-7 object-contain" />
                  </div>
                ) : (
                  <div className="w-9 h-9 bg-white/20 rounded-xl flex items-center justify-center text-white font-bold text-base shrink-0">
                    {storeName ? storeName.charAt(0).toUpperCase() : 'R'}
                  </div>
                )}
                {(!isSidebarCollapsed || isMobileOpen) && (
                  <div>
                    <p className="font-bold text-white text-sm leading-none">{storeName}</p>
          ...
          {/* Navigation Section */}
          <div className="flex-1 overflow-y-auto px-3 py-3 pb-6 no-scrollbar">
            <div className="space-y-5">
              {navGroups.map((group, gIdx) => (
                <div key={gIdx}>
                  {(!isSidebarCollapsed || isMobileOpen) && (
                    <p className="text-[10px] font-semibold text-white/30 uppercase tracking-wider px-2 mb-1.5">{group.title}</p>
                  )}
          ...
          <div className="px-3 pb-4 border-t border-white/10 pt-3">
            <button 
              ...
              className={`w-full flex items-center ${isSidebarCollapsed && !isMobileOpen ? 'justify-center' : 'gap-3 px-3'} py-2.5 rounded-xl text-sm font-medium text-white/50 hover:bg-white/10 hover:text-rose-400 transition-all group relative`}
            >
              <LogOut size={15} className="shrink-0" />
              {(!isSidebarCollapsed || isMobileOpen) && <span>Sign Out</span>}
              {isSidebarCollapsed && !isMobileOpen && (
                <div className="absolute left-full ml-3 px-3 py-1.5 bg-slate-900 text-white text-xs rounded-xl opacity-0 group-hover:opacity-100 transition-all pointer-events-none whitespace-nowrap z-50 shadow-xl">
                  Sign Out
                </div>
              )}
            </button>
      ...
      <div className="flex-1 flex flex-col min-w-0 h-screen overflow-hidden">
        ...
        <header className="h-14 bg-white dark:bg-[#0c0c0c] border-b border-slate-100 dark:border-white/5 flex items-center px-4 lg:px-6 shrink-0 z-20 sticky top-0 transition-all gap-3">
          <button 
            onClick={() => { if (window.innerWidth < 1024) setIsMobileOpen(true); else setIsSidebarCollapsed(!isSidebarCollapsed); }} 
            className="w-9 h-9 flex items-center justify-center text-slate-500 hover:text-slate-900 hover:bg-slate-100 rounded-xl transition-all shrink-0"
          >
            <Menu size={18} />
          </button>
          ...
          <div className="flex items-center justify-between flex-1">
            <div className="hidden md:flex items-center gap-2">
               ...
               <p className="text-xs font-medium text-slate-500">{t("all_services_active")}</p>
            </div>
            ...
            <div className="flex items-center gap-2 ml-auto">
              ...
              <button 
                ...
                className="px-2.5 h-8 flex items-center gap-1.5 rounded-xl text-slate-500 text-xs font-medium hover:text-slate-900 hover:bg-slate-100 transition-all"
              >
                <Globe size={13} />
              ...
              <Link href="/admin/profile" className="flex items-center gap-2.5 pl-3 border-l border-slate-200 dark:border-white/10">
                <div className="text-right hidden md:block">
                   <p className="text-xs font-semibold text-slate-800 dark:text-white leading-none">{fullName || session.user?.email?.split('@')[0]}</p>
                   <p className="text-[10px] text-slate-400 mt-0.5">{getRoleBadge(role)}</p>
                </div>
                <div className="relative">
                  <div className="w-8 h-8 rounded-xl bg-primary text-white flex items-center justify-center font-semibold text-sm overflow-hidden">
                     {avatarUrl ? (
                  ...
                  <div className="absolute -top-0.5 -right-0.5 w-2.5 h-2.5 bg-emerald-500 rounded-xl border-2 border-white" />
                </div>
             </Link>
            </div>
          </div>
        ...
        {/* Main Content Area */}
        <main className="flex-1 overflow-y-auto px-4 py-5 lg:px-8 lg:py-6 transition-all relative no-scrollbar bg-[#f8fafc] dark:bg-[#050505]">
              ...
              >
                 {hasPermission(pathname) ? children : (
                   <div className="min-h-[60vh] flex flex-col items-center justify-center p-10 text-center">
                      <div className="w-24 h-24 bg-rose-500/10 rounded-xl flex items-center justify-center text-rose-500 mb-8 border border-rose-500/20 shadow-inner">
                      ...
                      <Link href="/admin" className="px-8 py-4 bg-primary text-white rounded-xl text-[10px] font-black uppercase tracking-[0.3em] shadow-xl shadow-primary/20 hover:scale-105 transition-all">
