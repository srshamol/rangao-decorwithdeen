    ...
    <div className="min-h-screen bg-[#FDFCF9] pb-24 font-sans">
      ...
      <main className="container mx-auto max-w-7xl px-4 py-8">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
          <div className="lg:col-span-6 space-y-6">
            <div className="relative aspect-square rounded-xl overflow-hidden bg-white border border-slate-100 group">
              ...
              {discount > 0 && <div className="absolute top-4 left-4 bg-[#064E3B] text-white font-bold text-xs px-3 py-1.5 rounded-xl z-10">{discount}% OFF</div>}
              <button onClick={() => setActiveImage(prev => prev > 0 ? prev - 1 : images.length - 1)} className="absolute left-4 top-1/2 -translate-y-1/2 w-10 h-10 bg-white rounded-xl flex items-center justify-center shadow-md hover:bg-slate-50 transition-all z-20"><ChevronLeft size={20} /></button>
              <button onClick={() => setActiveImage(prev => prev < images.length - 1 ? prev + 1 : 0)} className="absolute right-4 top-1/2 -translate-y-1/2 w-10 h-10 bg-white rounded-xl flex items-center justify-center shadow-md hover:bg-slate-50 transition-all z-20"><ChevronRight size={20} /></button>
              <div className="absolute bottom-4 right-4 bg-black/50 text-white text-[10px] font-bold px-2 py-1 rounded-xl z-10">{activeImage + 1} / {images.length}</div>
            </div>
            <div className="flex gap-2.5 overflow-x-auto pb-2 scrollbar-hide">
              {images.map((img: string, i: number) => (
                <button key={i} onClick={() => setActiveImage(i)} className={`relative w-20 h-20 rounded-xl overflow-hidden border-2 shrink-0 transition-all ${activeImage === i ? 'border-primary' : 'border-transparent'}`}><img src={img} alt="" className="w-full h-full object-cover" /></button>
              ))}
            </div>
          </div>
          ...
          <div className="lg:col-span-6 space-y-8">
            <div className="space-y-4">
              <div className="flex flex-wrap gap-4 items-center"><div className="px-3 py-1 bg-emerald-50 text-[#064E3B] rounded-xl text-xs font-bold border border-emerald-100">বেস্ট সেলার</div><div className="flex items-center gap-2 text-slate-600 text-xs font-bold"><TrendingUp size={16} className="text-orange-500 fill-orange-500" />১৬০+ জন এই কম্বোটি কিনেছেন</div></div>
            ...
            <div className="flex items-center gap-4 flex-wrap">
              ...
              {discount > 0 && <div className="bg-[#DCFCE7] text-[#166534] px-4 py-2 rounded-xl text-xs font-bold border border-emerald-50">আপনি সেভ করছেন ৳ {product.old_price - product.price} ({discount}%)</div>}
            </div>

            {config.show_countdown !== false && (
              <div className="bg-white border border-slate-100 rounded-xl p-6 shadow-sm flex flex-col md:flex-row items-center justify-between gap-6 overflow-hidden relative group">
                <div className="absolute top-0 right-0 w-32 h-32 bg-primary/5 rounded-xl -mr-16 -mt-16 transition-transform group-hover:scale-150 duration-700" />
            ...
            <div className="space-y-3 pt-2">
               <Button onClick={handleOrderNow} className="w-full h-14 rounded-xl bg-[#064E3B] hover:bg-[#053F30] text-white font-bold text-lg gap-2 active:scale-[0.98] transition-all"><Zap size={20} className="fill-white" />{config.checkout_button_text || (bn ? "এখনি অর্ডার করুন" : "ORDER NOW")}</Button>
               <Button onClick={handleAddToCart} variant="outline" className="w-full h-14 rounded-xl border border-slate-200 hover:bg-slate-50 font-bold text-lg gap-2 text-slate-700 transition-all"><ShoppingCart size={20} />{bn ? "কার্টে যোগ করুন" : "ADD TO CART"}</Button>
            </div>
          </div>
        </div>
      ...
      {config.show_sticky_footer !== false && (
        <div className="fixed bottom-0 left-0 right-0 z-50 bg-white/95 backdrop-blur-md border-t border-slate-100 p-3 md:hidden flex gap-3 shadow-[0_-10px_40px_rgba(0,0,0,0.08)]">
           <Button onClick={handleOrderNow} className="flex-1 h-14 rounded-xl bg-[#064E3B] hover:bg-[#064E3B]/90 text-white font-black text-lg gap-2 shadow-xl shadow-emerald-900/10 active:scale-95 transition-all"><Zap size={20} className="fill-white" />{config.checkout_button_text || (bn ? "অর্ডার করুন" : "ORDER NOW")}</Button>
           <Button onClick={handleAddToCart} variant="outline" className="w-14 h-14 rounded-xl border-2 border-slate-100 hover:bg-slate-50 flex items-center justify-center p-0 text-slate-700"><ShoppingCart size={24} /></Button>
        </div>
      )}

      <CheckoutModal isOpen={isCheckoutOpen} onClose={() => setIsCheckoutOpen(false)} directCheckout={false} fixedProduct={product} />
      <div className="fixed top-0 right-0 w-[500px] h-[500px] bg-primary/5 blur-[150px] -z-10 rounded-xl pointer-events-none" />
      <div className="fixed bottom-0 left-0 w-[500px] h-[500px] bg-emerald-500/5 blur-[150px] -z-10 rounded-xl pointer-events-none" />
    </div>
  );
}
