...
export function HomeClient({ initialProducts, initialSettings }: HomeClientProps) {
  ...
  const DEFAULT_ORDER = [
    { id: "hero", type: 'hero', active: true, mobile: true, desktop: true },
    { id: "categories", type: 'categories', active: true, mobile: true, desktop: true },
    { id: "promo", type: 'promo_banners', active: true, mobile: true, desktop: true },
    { id: "featured", type: 'featured_products', active: true, mobile: true, desktop: true },
    { id: "trust", type: 'trust_metrics', active: true, mobile: true, desktop: true },
    { id: "why", type: 'why_choose', active: true, mobile: true, desktop: true },
    { id: "combo", type: 'combo', active: true, mobile: true, desktop: true },
    { id: "gallery", type: 'gallery', active: true, mobile: true, desktop: true },
    { id: "quote", type: 'islamic_quote', active: true, mobile: true, desktop: true },
    { id: "reviews", type: 'reviews', active: true, mobile: true, desktop: true },
    { id: "cta", type: 'cta', active: true, mobile: true, desktop: true },
  ];

  const sectionsToRender = hConfig.homepage_sections?.length > 0 ? hConfig.homepage_sections : DEFAULT_ORDER;

  return (
    <div className="pb-20 lg:pb-0 flex flex-col">
      {sectionsToRender.map((section: any, index: number) => {
        // Support both legacy 'active' and new 'enabled' property
        const isEnabled = section.enabled !== undefined ? section.enabled : section.active;
        if (!isEnabled) return null;
        
        let displayClass = "";
        const vis = section.visibility || { desktop: true, mobile: true };
        
        if (!vis.mobile && vis.desktop) displayClass = "hidden md:block";
        else if (vis.mobile && !vis.desktop) displayClass = "block md:hidden";
        else if (!vis.mobile && !vis.desktop) return null;

        return (
          <div key={section.id || `section-${index}`} className={displayClass}>
            {section.type === 'hero' && <Hero />}
            
            {section.type === 'categories' && activeSettings?.show_categories !== false && (
              <motion.div
            ...
            {section.type === 'featured_products' && activeSettings?.show_featured !== false && (
            ...
            {section.type === 'trust_metrics' && (
              <section className="px-4 md:px-6 py-12 md:py-20 bg-white">
                <div className="container mx-auto max-w-7xl">
                  <div className="bg-slate-50/50 rounded-xl py-8 px-4 grid grid-cols-2 md:grid-cols-4 items-center justify-between gap-y-10 md:gap-0 border border-slate-100">
            ...
            {section.type === 'combo' && activeSettings?.show_combo !== false && (
              <motion.div
                initial={{ opacity: 0, y: 40 }}
                whileInView={{ opacity: 1, y: 0 }}
                viewport={{ once: true, margin: "-100px" }}
            ...
            {section.type === 'gallery' && activeSettings?.show_gallery !== false && (
              <motion.div
                initial={{ opacity: 0, y: 40 }}
                whileInView={{ opacity: 1, y: 0 }}
                viewport={{ once: true, margin: "-100px" }}
            ...
            {section.type === 'reviews' && activeSettings?.show_reviews !== false && (
              <motion.div
                initial={{ opacity: 0, y: 40 }}
                whileInView={{ opacity: 1, y: 0 }}
                viewport={{ once: true, margin: "-100px" }}
              ...
              <section className="py-12 md:py-24 px-4 md:px-6 bg-white">
                <div className="max-w-7xl mx-auto">
                  <motion.div
                    ...
                    className="relative overflow-hidden rounded-xl md:rounded-xl bg-primary border border-primary-foreground/10 shadow-2xl p-8 md:p-24 group"
                  >
